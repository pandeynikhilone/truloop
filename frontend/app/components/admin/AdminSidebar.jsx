...
export default function AdminSidebar() {
    ...
    return (
        <>
            {/* Mobile Header */}
            <div className="lg:hidden bg-black text-white p-4 flex items-center justify-between sticky top-0 z-50">
            ...
            <aside className={`w-64 bg-black text-white min-h-screen flex flex-col p-6 fixed left-0 top-0 z-40 transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <div className="mt-auto">
                    <Link href="/">
                        <span className="block px-4 py-3 text-sm font-medium text-gray-400 hover:text-white hover:bg-white/10 rounded-lg">
                            Exit to Store
                        </span>
                    </Link>
                </div>
